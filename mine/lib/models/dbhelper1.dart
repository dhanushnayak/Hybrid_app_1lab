...
class DatabaseHelper {
  ...
  void _createDb(Database db, int newVersion) async {
    await db.execute(
        'CREATE TABLE $todoTable($colId INTEGER PRIMARY KEY AUTOINCREMENT,$colUser TEXT NOT NULL, $colTitle TEXT, '
  ...
  Future<List<Map<String, dynamic>>> getTodoMapList() async {
    ...
    // var result = await db.rawQuery('SELECT * FROM $todoTable where usn = '"$user"' , order by $colTitle ASC');
  ...
  Future<List<Todo>> getTodoList(dynamic user) async {
    Database db = await this.database;
    var todoMapList = await db.query(todoTable); // Get 'Map List' from database
    int count =
